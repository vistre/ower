 ...
 * 对应用户提供的 YAML 完整配置
 * 优化版：CDN 加速、DNS 精细化、Meta 内核特性开启、NTP 修正
 */
...
function main(config) {
  ...
  // ⚡️ [优化] 开启 TCP 并发连接，降低延迟
  config["tcp-concurrent"] = true;
  
  // ⚡️ [优化] 模拟 Chrome 指纹，防 TLS 指纹识别
  config["global-client-fingerprint"] = "chrome";

  // ⚡️ [优化] 开启 GeoData 模式，自动更新 IP 库
  ...
  // ==============================================================================
  // 2. 高级 DNS/TUN/Sniffer 配置
  // ==============================================================================
  config["lan-allowed-ips"] = [
    "0.0.0.0/0",
    "::/0",
    "127.0.0.1/8"
  ];
  ...
  config["dns"] = {
    ...
    // ⚡️ [优化] DNS 分流策略精细化
    // 增加 apple-cn, google-cn, 国内游戏 强制走国内 DNS，解决下载慢/登录慢
    "nameserver-policy": {
      "geosite:cn,private,apple-cn,google-cn,category-games@cn": [
        "tls://dns.alidns.com",
        "tls://dot.pub"
      ],
      "geosite:!cn,!private": [
  ...
  // ⚡️ [优化] 将 cdn.jsdelivr.net 替换为 fastly.jsdelivr.net (国内连接更稳)
  const providersDef = {
    ...
    // ACL4SSR 也尝试替换为 fastly 源，如果 ACL4SSR 没有同步到 fastly，保持原样或用 ghproxy
  ...
  for (const [key, val] of Object.entries(providersDef)) {
    myRuleProviders[key] = { ...ruleProviderBase, ...val };
  }
  ...
  // 定义排除关键词 (正则)
  const excludeFilter = "(?i)官网|官網|套餐|流量|测试|test|订阅|更新|维护|暂停|通知|超时|到期|剩余|重置|(([2-9]|\\d{2,})(\\.\\d+)?|1\\.[1-9])\\s*(x|×|倍)|(0\\.\\d+|0)(\\s)*(x|×|倍)|高倍|free|low|公益|低倍";
